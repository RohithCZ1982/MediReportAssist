"""
Patient Discharge Instructions Assistant - Colab Version
Main FastAPI application with RAG and Query Manager for Google Colab
"""
...
# Import Colab-compatible RAG system
from rag_system_colab import RAGSystemColab
from query_manager import QueryManager
from document_processor import DocumentProcessor

app = FastAPI(title="Patient Discharge Instructions Assistant (Colab)")
...
LLM_MODEL = os.getenv("LLM_MODEL", "gpt2")  # Default to small model
DB_PATH = os.getenv("DB_PATH", None)  # Use Drive path for persistence

# Initialize components
print("Initializing RAG System for Colab...")
rag_system = RAGSystemColab(
...
async def root():
    ...
    return {"message": "Patient Discharge Instructions Assistant API (Colab)", "docs": "/docs"}
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Save uploaded file (use Drive path if available)
        upload_dir = Path(DB_PATH) / "uploads" if DB_PATH else Path("uploads")
...
async def health_check():
    ...
    return {
        ...
        "llm_model": rag_system.llm_model_name,
        "documents_loaded": len(active_documents)
    }
